...
TEST_CASE("Emplace construction", "spsc_buffer") {
  using buf = gigafunction::spsc_buffer<
      std::pair<std::unique_ptr<int>, std::unique_ptr<int>>, 16>;
  buf b;
...
TEST_CASE("Spin policy", "spsc_buffer") {
  ...
  using buf =
      gigafunction::spsc_buffer<size_t, 16, local_rspinner, local_wspinner>;
  buf b;

  SECTION("Read spin triggers initial spin and spins") {
    auto t = std::thread([&b]() { REQUIRE(b.get() == 1); });
    // Initial spin is affected
    REQUIRE(timelimited_action(
        [&]() { return init_rspin.load(std::memory_order_relaxed) > 0; },
        std::chrono::milliseconds(100)));
    // Spin is affected
    REQUIRE(timelimited_action(
        [&]() { return rspin.load(std::memory_order_relaxed) > 0; },
        std::chrono::milliseconds(100)));
  ...
  SECTION("Write spin triggers initial spin and spins") {
    ...
    // Initial spin is affected
    REQUIRE(timelimited_action(
        [&]() { return init_wspin.load(std::memory_order_relaxed) > 0; },
        std::chrono::milliseconds(100)));
    // Spin is affected
    REQUIRE(timelimited_action(
        [&]() { return wspin.load(std::memory_order_relaxed) > 0; },
        std::chrono::milliseconds(100)));
