...
// Test various properties of the atomic bitmap and ensures it can be used with different
// bucket types (uint8_t, uint16_t, ...).
// NOTE: TestType is the name of the current type (named by Catch, the test-framework).
TEMPLATE_TEST_CASE("Atomic Bitmap", "atomicbitmap", uint8_t, uint16_t, uint32_t, uint64_t) {
  ...
  SECTION("Random single ranges set/get") {
    ...
    for (size_t i=0;i<32;i++) {
      ...
      // We cheat by clearing the backing store under the feet of BitmapSectionBase
  ...
  SECTION("Error cases with out of bounds access") {
    ...
    REQUIRE_THROWS_AS(bs.is_set(capacity), ErrorExit);
    REQUIRE_THROWS_AS(bs.set(capacity), ErrorExit);
    REQUIRE_THROWS_AS(bs.set_range(capacity-2, 3), ErrorExit);
  }
}
}