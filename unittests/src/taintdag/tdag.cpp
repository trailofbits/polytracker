...
namespace taintdag {
  TEST_CASE("Test basic TDAG construction", "[Integration]") {
    ...
    SECTION("Labels") {
      ...
      tdg.section<SourceLabelIndexSection>().set_range(BitIndex{test_range.first}, BitCount{length});
      REQUIRE(tdg.section<SourceLabelIndexSection>().size() > 0);

      // label union represents a step in the progression of taint
      auto test_union = tdg.section<Labels>().union_taint(test_range.first, test_range.second);
