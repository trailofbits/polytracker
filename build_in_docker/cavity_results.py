...
def main():
    ...
    parser.add_argument("--results", "-r", type=Path, required=True, help="Results directory containing all the *-verification.json files.")
    ...
    print(f"Accumulated file size: {processed_bytes}")
    print(f"Avg. file size {processed_bytes/file_count:.2f}")

    print(f"Total cavity byte count: {cavity_bytes}")
    print(f"Total non-cavity byte count (sampled): {noncavity_bytes}")
    ...
    print(f"Cavity mutations not affecting output {100.0*cavity_checksum_eq/cavity_bytes:.2f}%")
    print(f"Cavity bytes failing to produce output {100.0*cavity_no_output/cavity_bytes:.2f}%")
    print(f"Non-cavity  mutations not affecting output {100.0*noncavity_checksum_eq/noncavity_bytes:.2f}%")
    print(f"Non-cavity mutations affecting output {100.0*(noncavity_checksum_diff + noncavity_no_output)/noncavity_bytes:.2f}%")
