...
namespace taintdag {
  ...
  public:
    ...
    // label_t read_label_func(sink_offset_t), produces a label for each sink_offset in range
    // [file_offset, file_offset+length)
    template<typename F>
    void log_range(sink_index_t file, sink_offset_t file_offset, size_t length, F &&read_label_func) {
    ...
    void log_single(sink_index_t file, sink_offset_t file_offset, label_t label) {
      const size_t required_len = sizeof(file) + sizeof(file_offset) + sizeof(label);

      auto write_offset = offset_.fetch_add(required_len, std::memory_order_relaxed);
