...
-------------------------| [FDMappingHdr0][FDMappingHdr1]
/path/to/file1\0/path/to0\0 ^               ^ |               | name_offset1
-------------------------------               | name_offset0
--------------------------------------------------------------|


In theory, one wouldn't need the name_len field, it could be computed from the
start of the previous FDMappingHdr name_offset. To ease processing, it is
included anyway. N.b. name_offset is from the start of the memory allocated for
FDMappings, not the current FDMappingHdr.
*/
...
public:
  ...
  // The only reason this could fail is if there is no space left (either
  // storage or source_index)
  std::optional<index_t>
  add_mapping(int fd, std::string_view name,
  ...
  std::optional<std::pair<index_t, std::optional<taint_range_t>>>
  mapping_idx(int fd) const;
