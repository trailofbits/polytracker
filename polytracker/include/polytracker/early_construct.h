...
// The macros and template methods used have been tuned to give equal performance as
// a global variable (at least in -O3). Invoking get typically results in a lea of
// symbol address. This is equal to having a global variable.
...
T& get(typename std::aligned_storage<sizeof(T), alignof(T)>::type& src)
{
...
__attribute__((always_inline))
void construct(typename std::aligned_storage<sizeof(T), alignof(T)>::type& dst, Args&&... args) {
...
__attribute__((always_inline)) \
static auto& get_##NAME() { return early_construct_details::get<TYPE>(NAME); }
...
// Declare a variable named NAME, which is suitable to hold and early-constructed
// type TYPE. Provide additional getters for the value.
// Before getters can be used, the variable must be initialized via the
