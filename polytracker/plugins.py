...
class AbstractCommand(Plugin):
    ...
    def __init__(self, argument_parser: ArgumentParser, parent: Plugin | None = None):
...
def _lookup_class_property(name: str, bases: Iterable[type], clsdict: Mapping[str, Any]) -> Any:
    if name in clsdict:
        return clsdict[name]
    for base in bases:
        try:
    ...
    def __init__(cls, name, bases, clsdict):
        if not isabstract(cls) and name not in (
            ...
            if not has_parent_type:
                ...
                raise TypeError(
                    f"{basename} {cls.__name__}'s `parent_type` of {clsdict['parent_type']!r} does not extend off of Command"
                )
    ...
    def __init_subclass__(cls, **kwargs):
        if not isabstract(cls):
            ...
            if cls in cls.parent_command_type.extension_types:
                raise TypeError(
                    f"CommandExtension {cls.__name__} is already registered to Command {cls.parent_command_type.__name__}"
                )
    ...
    def __init_subclass__(cls, **kwargs):
        if not isabstract(cls):
            ...
            if cls in cls.parent_command_type.subcommand_types:
                raise TypeError(
                    f"Subcommand {cls.__name__} is already registered to Command {cls.parent_command_type.__name__}"
                )
