...
function(add_compiler_rt_object_libraries name)
	...
	if(APPLE)
		...
		foreach(arch ${LIB_ARCHS})
			...
			set(libnames ${libnames} ${libname})
			set(extra_cflags_${libname} "-fPIC" ${TARGET_${arch}_CFLAGS})
...
function(add_compiler_rt_custom_cxx name type)
	...
	if(NOT CAN_TARGET_${X86_64})
		message(FATAL_ERROR "Architecture ${X86_64} can't be targeted")
		return()
	endif()
	if(type STREQUAL "STATIC")
		set(libname "${name}-${X86_64}")
		set_output_name(output_name_${libname} ${name} ${X86_64})
	else()
		set(libname "${name}-dynamic-${X86_64}")
		set(extra_cflags_${libname} ${TARGET_${X86_64}_CFLAGS} ${LIB_CFLAGS})
		set(extra_link_flags_${libname} ${TARGET_${X86_64}_LINK_FLAGS} ${LIB_LINK_FLAGS})
		set_output_name(output_name_${libname} ${name} ${X86_64})
	endif()
	set(sources_${libname} ${LIB_SOURCES})
	format_object_libs(sources_${libname} ${X86_64} ${LIB_OBJECT_LIBS})
	set(libnames ${libnames} ${libname})
		#Add extra flags to target libc++
		#link_directories("../dfsan_private_headers/lib")
		#include_directories("../dfsan_private_headers/include/c++/v1")
		set(extra_cflags_${libname} "-fPIC" ${TARGET_${X86_64}_CFLAGS} ${LIB_CFLAGS})
	#Add link dir with the ABI
	#set(POLY_LIB_PATH "../dfsan_private_headers/lib")
	#set(extra_link_flags_${libname} "-Wl,--whole-archive ${POLY_LIB_PATH}/libc++.a" "${POLY_LIB_PATH}/libc++.a -Wl,--no-whole-archive")
	if(NOT libnames)
		message(FATAL_ERROR "NO LIBNAMES")
		return()
	endif()
	...
	message(STATUS "LIBBAMES ARE ${libnames}")
	foreach(libname ${libnames})
		...
		message(STATUS "Adding extra compile flags: ${extra_cflags_${libname}}")
		set_target_link_flags(${libname} ${extra_link_flags_${libname}})
		message(STATUS "Adding extra link flags: ${extra_link_flags_${libname}}")
		set_property(TARGET ${libname} APPEND PROPERTY
			COMPILE_DEFINITIONS ${LIB_DEFS})
		...
		message(STATUS "GOING FOR THE LINK") 
		
		if(LIB_LINK_LIBS)
			message(STATUS "link libs are: ${LIB_LINK_LIBS}")
