...
EXT_C_FUNC ssize_t __dfsw_read(int fd, void *buff, size_t size,
  ...
  if (isTrackingSource(fd)) {
    if (ret_val > 0) {
      taintData(fd, (char *)buff, read_start, ret_val);
    }
    *ret_label = 0;
  } else {
    *ret_label = 0;
...
EXT_C_FUNC ssize_t __dfsw_pread(int fd, void *buf, size_t count, off_t offset,
  ...
  if (isTrackingSource(fd)) {
    if (ret > 0) {
      taintData(fd, (char *)buf, offset, ret);
    }
    *ret_label = 0;
  } else {
    *ret_label = 0;
...
EXT_C_FUNC size_t __dfsw_fread(void *buff, size_t size, size_t count, FILE *fd,
  ...
  if (isTrackingSource(fd)) {
    if (ret > 0) {
      ...
      taintData(fd, (char *)buff, offset, ret * size);
    }
    *ret_label = 0;
  } else {
#ifdef DEBUG_INFO
...
#endif
  if (c != EOF && isTrackingSource(fd)) {
    *ret_label = createReturnLabel(
        offset, getSourceName(fd));
  }
  return c;
}
...
#endif
  if (ret) {
    taintData(stdin, str, offset, strlen(ret));
    *ret_label = str_label;
  } else {
    *ret_label = 0;
  }
...
#endif
  if (ret > 0 && isTrackingSource(fd)) {
    taintData(fd, *lineptr, offset, ret);
  }
  *ret_label = 0;
  return ret;
}
...
EXT_C_FUNC void *__dfsw_mmap(void *start, size_t length, int prot, int flags,
  ...
  if (ret && isTrackingSource(fd)) {
    taintData(fd, (char *)ret, offset, length);
  }
  *ret_label = 0;
  return ret;
}
