...
EXT_C_FUNC int __dfsw_open(const char *path, int oflags, dfsan_label path_label,
	...
	taint_manager->logFunctionEntry("__dfsw_open");
	va_list args;
...
EXT_C_FUNC int __dfsw_openat(int dirfd, const char *path, int oflags,
		...
		...) {
	taint_manager->logFunctionEntry("__dfsw_openat");
	va_list args;
	va_start(args, ret_label);
...
EXT_C_FUNC FILE *__dfsw_fopen64(const char *filename, const char *mode,
		...
		dfsan_label *ret_label) {
	taint_manager->logFunctionEntry("__dfsw_fopen64");
...
EXT_C_FUNC FILE *__dfsw_fopen(const char *filename, const char *mode,
		dfsan_label fn_label, dfsan_label mode_label,
		dfsan_label *ret_label) {
	taint_manager->logFunctionEntry("__dfsw_fopen");
	FILE *fd = fopen(filename, mode);
#ifdef DEBUG_INFO
...
EXT_C_FUNC int __dfsw_close(int fd, dfsan_label fd_label,
		dfsan_label *ret_label) {
	taint_manager->logFunctionEntry("__dfsw_close");
	int ret = close(fd);
#ifdef DEBUG_INFO
...
EXT_C_FUNC int __dfsw_fclose(FILE *fd, dfsan_label fd_label,
		dfsan_label *ret_label) {
	taint_manager->logFunctionEntry("__dfsw_fclose");
	int ret = fclose(fd);
#ifdef DEBUG_INFO
...
EXT_C_FUNC ssize_t __dfsw_read(int fd, void *buff, size_t size,
	...
	taint_manager->logFunctionEntry("__dfsw_read");
	...
	if (taint_manager->isTracking(fd)) {
		...
		//FIXME create a taint label here I think
		//FIXME this is a length, testcase for this.
		*ret_label = 0;
	} else {
		*ret_label = 0;
	}
...
EXT_C_FUNC ssize_t __dfsw_pread(int fd, void *buf, size_t count, off_t offset,
		...
		dfsan_label *ret_label) {
	taint_manager->logFunctionEntry("__dfsw_pread");
	ssize_t ret = pread(fd, buf, count, offset);
	if (taint_manager->isTracking(fd)) {
		if (ret > 0) {
			taint_manager->taintData(fd, (char *)buf, offset, ret);
		}
		//FIXME create label here i think
		*ret_label = 0;
	} else {
		*ret_label = 0;
	}
...
EXT_C_FUNC ssize_t __dfsw_pread64(int fd, void *buf, size_t count, off_t offset,
		...
		dfsan_label offset_label,
		dfsan_label *ret_label) {
	taint_manager->logFunctionEntry("__dfsw_pread64");
#ifdef DEBUG_INFO
...
EXT_C_FUNC size_t __dfsw_fread(void *buff, size_t size, size_t count, FILE *fd,
		...
		dfsan_label *ret_label) {
	taint_manager->logFunctionEntry("__dfsw_fread");
...
EXT_C_FUNC size_t __dfsw_fread_unlocked(void *buff, size_t size, size_t count,
		...
		dfsan_label *ret_label) {
	taint_manager->logFunctionEntry("__dfsw_fread_unlocked");
	long offset = ftell(fd);
...
EXT_C_FUNC int __dfsw_fgetc(FILE *fd, dfsan_label fd_label,
		dfsan_label *ret_label) {
	taint_manager->logFunctionEntry("__dfsw_fgetc");
	long offset = ftell(fd);
	int c = fgetc(fd);
	//FIXME
	*ret_label = 0;
	...
	if (c != EOF && taint_manager->isTracking(fd)) {
		dfsan_label new_label = taint_manager->createReturnLabel(
				offset, taint_manager->getTargetInfo(fd)->target_name);
	     *ret_label = new_label;
#ifdef DEBUG_INFO
	     //FIXME
	     dfsan_set_label(new_label, ret_label, TAINT_GRANULARITY);

	     std::cout << "SET LABEL AND RET_LABEL IS: " << new_label << std::endl;
#endif
	}
	taint_manager->logFunctionExit();
	return c;
...
EXT_C_FUNC int __dfsw_fgetc_unlocked(FILE *fd, dfsan_label fd_label,
		dfsan_label *ret_label) {
	taint_manager->logFunctionEntry("__dfsw_fgetc_unlocked");
	long offset = ftell(fd);
...
EXT_C_FUNC int __dfsw__IO_getc(FILE *fd, dfsan_label fd_label,
		dfsan_label *ret_label) {
	taint_manager->logFunctionEntry("__dfsw_IO_getc");
	long offset = ftell(fd);
	int c = getc(fd);
	//FIXME
	*ret_label = 0;
...
EXT_C_FUNC int __dfsw_getchar(dfsan_label *ret_label) {
	taint_manager->logFunctionEntry("__dfsw_getchar");
...
EXT_C_FUNC char *__dfsw_fgets(char *str, int count, FILE *fd,
	...
	taint_manager->logFunctionEntry("__dfsw_fgets");
	long offset = ftell(fd);
