...
TEST_CASE("Encoding decoding") {
  ...
  for (auto i=0;i<100000;i++) {
    auto choice = rand_limit(3u);
    INFO("Choice is " << choice);
    bool affects_control_flow = rand_limit(2u);
    source_offset_t ofs = rand_limit(max_source_offset+1u);
    source_index_t idx = rand_limit(max_source_index+1u);
    label_t label1 = rand_limit(max_label + 1u);
    label_t label2 = rand_limit(max_label + 1u);
