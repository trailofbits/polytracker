#ifndef POLYTRACKER_TAINTDAG_TEST_HELPERS_HPP
...
inline std::pair<SourceTaint, label_t> random_source_taint(source_index_t max_source  = max_source_index, source_offset_t max_offset = max_source_offset, label_t lblmax = max_label) {
  return {SourceTaint(rand_limit(max_source), rand_limit(max_offset)), lbl_inrange(1, lblmax)};
}
...
inline std::pair<UnionTaint, label_t> random_union_taint(label_t maxlbl = max_label-1) {
  ...
  return {UnionTaint{l1, l2}, lbl_inrange(hilbl+1, std::min(hilbl+8, max_label))};
}
...
inline std::pair<RangeTaint, label_t> random_range_taint(label_t maxlbl = max_label-1) {
  ...
  if (first == last)
    last++;
  return {RangeTaint{first, last}, lbl_inrange(last+1, std::min(last + 8, max_label))};
}



// Create a random taint value/label pair
inline std::pair<Taint, label_t> rand_taint(label_t maxlabel=max_label, source_index_t max_source = max_source_index, source_offset_t max_offset = max_source_offset) {
  auto i = rand_limit(3);
  switch(i) {
    case 0: return random_source_taint(max_source, max_offset, maxlabel);
    case 1: return random_union_taint(maxlabel);
    default: return random_range_taint(maxlabel);
  }
}
