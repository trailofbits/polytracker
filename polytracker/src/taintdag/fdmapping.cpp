...
namespace taintdag {
  ...
  FDMapping::FDMapping(char *begin, char *end)
   : begin_{begin}, end_{end} {
     // TODO (hbrodin): Add a check to ensure begin is aligned by FDMapping::alignat.
  }

  // The only reason this could fail is if there is no space left (either storage or source_index)
  std::optional<FDMapping::index_t> FDMapping::add_mapping(int fd, std::string_view name, std::optional<taint_range_t> preallocated_labels) {
  ...
  std::optional<std::pair<FDMapping::index_t, std::optional<taint_range_t>>> FDMapping::mapping_idx(int fd) const {
  ...
  std::optional<FDMapping::length_type> FDMapping::write_name(std::string_view name) {
