...
std::optional<index_t> Functions::add_mapping(uint32_t function_id,
  ...
  std::optional<StringTable::offset_t> maybe_name_offset =
      string_table.add_string(function_name);
  ...
  if (!maybe_ctx.has_value()) {
    spdlog::error("Could not write Function {0} with id {1:d}, string table "
                  "ofs {2:d} to the tdag functions section",
                  function_name, function_id, maybe_name_offset.value());
    return {};
  }
