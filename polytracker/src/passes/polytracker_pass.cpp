...
void PolytrackerPass::visitBranchInst(llvm::BranchInst &BI) {
  if (BI.isUnconditional()) {
    ...
    // most sizeof(shadow_type)?? To be able to pass it as first ar to
      ...
      for (auto &bb : func) {
        for (auto &inst : bb) {
          if (auto *BI = llvm::dyn_cast<llvm::BranchInst>(&inst)) {
            ...
            // %81 = call %struct._IO_FILE* @__dfsw_fopen(i8* %80, i8*
            // getelementptr inbounds ([2 x i8], [2 x i8]* @.str.1, i64 0, i64
            // 0),
            //                                            i32 zeroext %79, i32
            //                                            zeroext 0, i32*
            //                                            %labelreturn)
