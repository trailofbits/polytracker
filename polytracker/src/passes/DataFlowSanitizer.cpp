...
llvm::createDataFlowSanitizerPass(const std::vector<std::string> &ABIListFiles,
                                  void *(*getArgTLS)(),
                                  void *(*getRetValTLS)()) {
  std::cout << "Creating special case list!" << std::endl;
...
bool DataFlowSanitizer::runOnModule(Module &M) {
  ...
  for (Function *i : FnsToInstrument) {
    ...
    // branch.
    std::cout << "About to split a block" << std::endl;
    ...
    for (BasicBlock *curr_bb : BBList) {
      ...
      // Add a callback for BB entry
      {
        ...
        std::cout << "Making block entry" << std::endl;
        IRB.CreateCall(DFSanEntryBBFn, {FuncName, FuncIndex, BBIndex, BBType});
        std::cout << "Made block entry" << std::endl;
      }
      while (true) {
        Instruction *Next = Inst->getNextNode();
        bool IsTerminator = isa<TerminatorInst>(Inst);
  ...
  std::cout << "DONE RUN ON MODULE" << std::endl;
...
static void registerAflDFSanPass(const PassManagerBuilder &,
  ...
  std::cout << "Adding new pass?" << std::endl;
