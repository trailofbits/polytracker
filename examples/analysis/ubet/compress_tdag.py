...
def copy_section(fin, fout, section_in: taint_dag.TDSectionMeta, section_out: taint_dag.TDSectionMeta):
  ...
  os.copy_file_range(fin.fileno(), fout.fileno(), section_in.size, section_in.offset, section_out.offset)


def compact_section(starting_offset:int, section_in: taint_dag.TDSectionMeta) -> taint_dag.TDSectionMeta:
...
def main():
  parser = ArgumentParser(prog="compress_tdag", description="Compress a sparse tdag file")
  parser.add_argument("-i", "--input", help="Sparse input (source) tdag file", type=Path, required=True)
  parser.add_argument("-o", "--output", help="Dense output (destination) tdag file", type=Path, required=True)
